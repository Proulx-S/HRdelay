function [N,crossValDelay,binCent,crossValDelayX,delayX,binCent2] = runCrossValDelay2(dOrig,p,verbose)
...
for kInd = 1:length(unique(d.crossVal))
    ...
    tmpTr1 = [];
    for i = 1:p.avBin
        tmpTe1 = [tmpTe1 teDelayDiff(kInd,logical(binInd(i,:)))];
        tmpTr1 = [tmpTr1 trDelayDiff(kInd,logical(binInd(i,:)))];
    end
    tmpTe2 = [];
    tmpTr2 = [];
    for i = length(N(kInd,:))-p.avBin+1:length(N(kInd,:))
        tmpTe2 = [tmpTe2 teDelayDiff(kInd,logical(binInd(i,:)))];
        tmpTr2 = [tmpTr2 trDelayDiff(kInd,logical(binInd(i,:)))];
    end
    
    crossValDelayX(kInd,:) = [circ_mean(tmpTe1,[],2) crossValDelay(kInd,p.avBin+1:end-p.avBin) circ_mean(tmpTe2,[],2)];
    delayX(kInd,:) = [circ_mean(tmpTr1,[],2) delay(kInd,p.avBin+1:end-p.avBin) circ_mean(tmpTr2,[],2)];
end
...
for i = 1:size(delay,2)
    tmp = delay(~isnan(delay(:,i)),i);
    if isempty(tmp)
        delay2(1,i) = nan;
    else
        delay2(1,i) = circ_mean(tmp);
    end
end
delay = delay2;


for i = 1:size(crossValDelayX,2)
    tmp = crossValDelayX(~isnan(crossValDelayX(:,i)),i);
    if isempty(tmp)
        crossValDelayX2(1,i) = nan;
    else
        crossValDelayX2(1,i) = circ_mean(tmp);
    end
end
crossValDelayX = crossValDelayX2;

for i = 1:size(delayX,2)
    tmp = delayX(~isnan(delayX(:,i)),i);
    if isempty(tmp)
        delayX2(1,i) = nan;
    else
        delayX2(1,i) = circ_mean(tmp);
    end
end
delayX = delayX2;
