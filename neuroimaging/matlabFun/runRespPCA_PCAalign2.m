function [pcaRes,predRes] = runRespPCA_PCAalign2(dOrig,p,verbose)
...
%% Do spatial PCA all
pcaRes.all.x = cat(3,mean(d.x(d.y==1,:,:),1),mean(d.x(d.y==2,:,:),1),mean(d.xNorm,1));
pcaRes.all.x = permute(pcaRes.all.x,[3 2 1]);
pcaRes.all.x_shift = mean(pcaRes.all.x,1);
pcaRes.all.x = pcaRes.all.x - repmat(pcaRes.all.x_shift,[size(pcaRes.all.x,1) 1]);
pcaRes.all.x_scale = std(pcaRes.all.x,1);
pcaRes.all.x = pcaRes.all.x./repmat(pcaRes.all.x_scale,[size(pcaRes.all.x,1) 1]);
[pcaRes.all.coeff,pcaRes.all.score,pcaRes.all.latent,~,~,~] = pca(pcaRes.all.x,'centered',0); %coeff: run x pcTime x vox
plotPCA2(pcaRes.all,p,3)
saveas(gca,fullfile(p.dataDirOut,[p.dataFileOut '__spatialPCAonHR_PCs.jpg']))

% tmp = pcaRes.all.score.*mean(pcaRes.all.x_scale)+mean(pcaRes.all.x_shift);

%test PC significance
latent = [];
for i = 1:100
    x = [];
    for ii = 1:size(pcaRes.all.x,2)
        x(:,ii) = pcaRes.all.x(randperm(size(pcaRes.all.x,1)),ii);
    end
    [~,~,latent(:,i),~,~,~] = pca(x,'centered',0); %coeff: run x pcTime x vox
end
figure('WindowStyle','docked');
plot(pcaRes.all.latent,'-o'); hold on
thresh = prctile(latent,[5 95],2);
errorbar((1:size(latent,1))',mean(latent,2),thresh(:,1)-mean(latent,2),thresh(:,2)-mean(latent,2),'r');
xlabel('PC')
ylabel('eigenvalue')
legend({'actual' 'permuted (mean +- 90% conf.int.)'})
title([p.subj '; ' p.preSelection '; scree plot of spatialPCAonHR'])
saveas(gca,fullfile(p.dataDirOut,[p.dataFileOut '__spatialPCAonHR_scree.jpg']))
...
pcaRes.all.R = eye(size(pcaRes.all.score,1)./3);
for i = 2:size(pcaRes.all.score,2);
    a1=pcaRes.all.score(end/3*0+1:end/3*1,i);
    a2=pcaRes.all.score(end/3*1+1:end/3*2,i);
    a3=pcaRes.all.score(end/3*2+1:end/3*3,i);
    a = mean(cat(2,a1,a2,a3),2);
    b1=pcaRes.all.score(end/3*0+1:end/3*1,1);
    b2=pcaRes.all.score(end/3*1+1:end/3*2,1);
    b3=pcaRes.all.score(end/3*2+1:end/3*3,1);
    b = mean(cat(2,b1,b2,b3),2);
    ...
    if any(ismember(i,sigPCind))
        pcaRes.all.R(:,:,i) = reflection( S, v );             % v = R*u
    else
        pcaRes.all.R(:,:,i) = eye(size(pcaRes.all.score,1)./3);
    end
end


%% Rotate PCs to first PC
pcaRes.all.scoreR = nan(size(pcaRes.all.score));
for i = 1:size(pcaRes.all.score,2)
%     if i==1
        pcaRes.all.scoreR(end/3*0+1:end/3*1,i) = pcaRes.all.R(:,:,i)*pcaRes.all.score(end/3*0+1:end/3*1,i);
        pcaRes.all.scoreR(end/3*1+1:end/3*2,i) = pcaRes.all.R(:,:,i)*pcaRes.all.score(end/3*1+1:end/3*2,i);
        pcaRes.all.scoreR(end/3*2+1:end/3*3,i) = pcaRes.all.R(:,:,i)*pcaRes.all.score(end/3*2+1:end/3*3,i);
%     else
%         pcaRes.all.scoreR(end/3*0+1:end/3*1,i) = pcaRes.all.R(:,:,2)*pcaRes.all.score(end/3*0+1:end/3*1,i);
%         pcaRes.all.scoreR(end/3*1+1:end/3*2,i) = pcaRes.all.R(:,:,2)*pcaRes.all.score(end/3*1+1:end/3*2,i);
%         pcaRes.all.scoreR(end/3*2+1:end/3*3,i) = pcaRes.all.R(:,:,2)*pcaRes.all.score(end/3*2+1:end/3*3,i);
%     end
end

%% Project back into original space
pcaRes.all.xR = pcaRes.all.scoreR*pcaRes.all.coeff';


% [pcaRes.all.coeff,pcaRes.all.score,pcaRes.all.latent,~,~,~] = pca(pcaRes.all.x,'centered',0); %coeff: run x pcTime x vox
% plotPCA(pcaRes.all,p,3)
% 
% figure('WindowStyle','docked');
...
pcaTimeRes.x = (pcaRes.all.xR(end/3*0+1:end/3*1,:) - pcaRes.all.xR(end/3*1+1:end/3*2,:))';
[pcaTimeRes.coeff,pcaTimeRes.score,pcaTimeRes.latent,~,~,pcaTimeRes.mu] = pca(pcaTimeRes.x,'centered',true);
%test PC significance
latent = [];
for i = 1:100
    x = [];
    for ii = 1:size(pcaTimeRes.x,2)
        x(:,ii) = pcaTimeRes.x(randperm(size(pcaTimeRes.x,1)),ii);
    end
    [~,~,latent(:,i),~,~,~] = pca(x,'centered',0); %coeff: run x pcTime x vox
end
figure('WindowStyle','docked');
plot(pcaTimeRes.latent,'-o'); hold on
thresh = prctile(latent,[5 95],2);
errorbar((1:size(latent,1))',mean(latent,2),thresh(:,1)-mean(latent,2),thresh(:,2)-mean(latent,2),'r');
xlabel('PC')
ylabel('eigenvalue')
legend({'actual' 'permuted (mean+-90%conf.int.)'})
title([p.subj '; ' p.preSelection '; scree plot of temporalPCAonHRdiff'])
saveas(gca,fullfile(p.dataDirOut,[p.dataFileOut '__temporalPCAonHRdiff_scree.jpg']))

figure('WindowStyle','docked');
plot(pcaTimeRes.coeff(:,1),'b'); hold on
plot(-pcaTimeRes.coeff(:,1),'--b');
plot(pcaTimeRes.coeff(:,2),'r');
plot(-pcaTimeRes.coeff(:,2),'--r');
plot(mean(cat(2,pcaRes.all.scoreR(end/3*0+1:end/3*1,1),pcaRes.all.scoreR(end/3*1+1:end/3*2,1),pcaRes.all.scoreR(end/3*2+1:end/3*3,1)),2)/100,'k')
legend({'PC1' '-PC1' 'PC2' '-PC2' 'HR PC1'})
title([p.subj '; ' p.preSelection '; temporalPCAonHRdiff'])
saveas(gca,fullfile(p.dataDirOut,[p.dataFileOut '__temporalPCAonHRdiff_PCs.jpg']))


%% Compute shape difference and do spatial PCA on that
pcaTimeRes.x = (pcaRes.all.xR(end/3*0+1:end/3*1,:) - pcaRes.all.xR(end/3*1+1:end/3*2,:));
[pcaTimeRes.coeff,pcaTimeRes.score,pcaTimeRes.latent,~,~,pcaTimeRes.mu] = pca(pcaTimeRes.x,'centered',true);
%test PC significance
latent = [];
...
ylabel('eigenvalue')
legend({'actual' 'permuted (mean+-90%conf.int.)'})
title([p.subj '; ' p.preSelection '; scree plot of spatialPCAonHRdiff'])
saveas(gca,fullfile(p.dataDirOut,[p.dataFileOut '__spatialPCAonHRdiff_scree.jpg']))

figure('WindowStyle','docked');
plot(pcaTimeRes.score(:,1),'b'); hold on
plot(-pcaTimeRes.score(:,1),'--b');
plot(pcaTimeRes.score(:,2),'r');
plot(-pcaTimeRes.score(:,2),'--r');
tmp = mean(cat(2,pcaRes.all.scoreR(end/3*0+1:end/3*1,1),pcaRes.all.scoreR(end/3*1+1:end/3*2,1),pcaRes.all.scoreR(end/3*2+1:end/3*3,1)),2);
plot(tmp,'k')
legend({'PC1' '-PC1' 'PC2' '-PC2' 'HR PC1'})
title([p.subj '; ' p.preSelection '; spatialPCAonHRdiff'])
saveas(gca,fullfile(p.dataDirOut,[p.dataFileOut '__spatialPCAonHRdiff_PCs.jpg']))


% %% Compute shape difference and do temporal ICA on that
% pcaTimeRes.x = (pcaRes.all.xR(end/3*0+1:end/3*1,:) - pcaRes.all.xR(end/3*1+1:end/3*2,:))';
% 
% [Zica, W, T, mu] = fastICA(pcaTimeRes.x',3,'negentropy',1);
% A = T \ W'; %x = A*s;
...
% 
% figure('WindowStyle','docked');
% plot(A(:,1)-mean(A(:,1))); hold on
% plot(-A(:,2)-mean(-A(:,2)));
% 
% figure('WindowStyle','docked');
% plot(W(1,:)-mean(W(1,:))); hold on
% plot(-W(2,:)-mean(-W(2,:)));
% 
% plot(W'); hold on
% tmp = mean(cat(2,pcaRes.all.scoreR(end/3*0+1:end/3*1,1),pcaRes.all.scoreR(end/3*1+1:end/3*2,1),pcaRes.all.scoreR(end/3*2+1:end/3*3,1)),2);
