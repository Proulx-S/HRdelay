...
[alpha, idx, w, warningFlag] = processInput(varargin{:});
...
function [alpha, idx, w, warningFlag] = processInput(varargin)
if length(varargin) == 5 && isempty(varargin{3}) && isempty(varargin{4})
    if isnumeric(varargin{5}) && varargin{5}
        warningFlag = 1;
    else
        warningFlag = 0;
    end
...
if length(varargin) == 4
    ...
    w = [w1; w2];
elseif length(varargin)==2 && sum(abs(round(varargin{2})-varargin{2}))>1e-5
    alpha1 = varargin{1}(:);
    alpha2 = varargin{2}(:);
    alpha = [alpha1; alpha2];
