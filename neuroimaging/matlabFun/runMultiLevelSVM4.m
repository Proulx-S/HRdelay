function [rTr,rTe] = runMultiLevelSVM4(dOrig,p)
% if 0
p.getPattern = true;
p.perm = p.doPerm;
...
for k = 1:length(dFold)
    ...
    if size(dFold{1}.yhat,2)>1
        %Store individual subfolds
        crossVal = unique(d.crossVal);
        if all(diff(dFold{k}.crossVal)==0)
            crossVal(crossVal==dFold{k}.crossVal(1)) = [];
        else
            error('XX')
        end
        indK = ~ismember(d.crossVal,dFold{k}.crossVal);
        indLabel1 = d.y==1;
        indLabel2 = d.y==2;
        for subK = 1:length(crossVal) %size(dFold{k}.yhat,2)
            indSubK = d.crossVal==crossVal(subK);
            indSubKLabel1 = dFold{k}.y==1;
            indSubKLabel2 = dFold{k}.y==2;
            
            r.pred(indK&indSubK&indLabel1,k) = dFold{k}.pred(indSubKLabel1,subK);
            r.yhat(indK&indSubK&indLabel1,k) = dFold{k}.yhat(indSubKLabel1,subK);
            r.pred(indK&indSubK&indLabel2,k) = dFold{k}.pred(indSubKLabel2,subK);
            r.yhat(indK&indSubK&indLabel2,k) = dFold{k}.yhat(indSubKLabel2,subK);
        end
    else
        %Subfolds non-exitent or already averaged
        ind = ismember(d.crossVal,dFold{k}.crossVal);
        if any(d.crossVal(ind)~=dFold{k}.crossVal) || any(d.y(ind)~=dFold{k}.y)
            error('XX')
        end
