...
for kInd = 1:length(unique(d.crossVal))
    ...
    [tr.delayCartr.x,tr.delayCarti.x] = pol2cart(tr.delayPol.x,ones(size(trMag)));    
    ...
    [tr.cartr_roiDelay.x,tr.carti_roiDelay.x] = pol2cart(wrapToPi(trPhase-repmat(phaseShift_roi,size(trPhase,1),1)),tr.magPol.x);
    ...
    [te.delayCartr.x,te.delayCarti.x] = pol2cart(te.delayPol.x,ones(size(teMag)));    
    ...
    [te.cartr_roiDelay.x,te.carti_roiDelay.x] = pol2cart(wrapToPi(tePhase-repmat(phaseShift_roi,size(tePhase,1),1)),te.magPol.x);
...
switch redDim
    case 'info'
        ...
        for i = 1:size(xTr,3)
            ...
            if p.subCrossVal
                newxTrSub = cat(2,newxTrSub,dTr_cur_cur{i}.yhatSub);
            end
            %Test
    ...
    case 'vox'
        ...
        for i = 1:size(xTr,3)
            ...
            dTe_cur_cur{i}.x = xTe(:,:,i);
            dTe_cur_cur{i} = testSVMatLevel(dTe_cur_cur{i},dTr_cur_cur{i});
%             dTe_cur_cur{i} = trainSVMatLevel(dTe_cur_cur{i},p);
            newxTe = cat(2,newxTe,dTe_cur_cur{i}.yhat);
        end
        %Store data
        dTr_cur.x = newxTr;
...
if p.getPattern
    ...
    if p.subCrossVal
        d.aSub = nan(length(crossVal),size(d.x,2));
        d.wSub = nan(length(crossVal),size(d.x,2));
    end
end
...
if p.subCrossVal
    ...
    for kInd = 1:length(crossVal)
        ...
        if p.getPattern
            [d.wSub(kInd,:),d.aSub(kInd,:)] = getAandW(dTr.svmStruct,dTr.x);
        end
    end
    d.wSub = mean(d.wSub,1);
    d.aSub = mean(d.aSub,1);
    d.crossVal = d.crossVal_orig;
end
...
if p.subCrossVal
    d.yhatSub = nan(size(d.y));
    d.predSub = nan(size(d.y));
    if p.getPattern
        d.aSub = nan(length(crossVal),size(d.x,2));
        d.wSub = nan(length(crossVal),size(d.x,2));
    end
...
if p.subCrossVal
    ...
    for kInd = 1:length(crossVal)
        ...
        dTr.svmStruct = svmtrain(dTr.y,dTr.x,['-s 0 -t 0 -c 1 -q']);
        %Test model
        [d.predSub(teInd), ~, d.yhatSub(teInd)] = svmpredict(dTe.y,dTe.x,dTr.svmStruct,'-q');
        %Extract model weigths
        if p.getPattern
            [d.wSub(kInd,:),d.aSub(kInd,:)] = getAandW(dTr.svmStruct,dTr.x);
        end
