function hitRate = runSVM_patAct8(d,p,verbose,rep)
...
%             k = length(d.crossVal)/2/str2double(p.split(1));
        case {'run','runRand','sess','8FoldsRunRand'}
            d.crossVal = defineCrossVal(d,p);
...
if ~all(strcmp(p.trainInfo,p.testInfo)) || p.doCrossInfo
    error('not meant for crossInfo')
end
if strcmp(p.k,'sess')
    error('not meant for between-session stuff')
end


[hitRate] = multiComb(dOrig,p,k,rep,verbose);
...
function [hitRate] = multiComb(dOrig,p,k,rep,verbose)
for fold = 1:k
    ...
    for subFold = 1:k-1
        [hitRate(fold,1),curDecision,teInd,fData,curw,cura] = trainNtest5(dSubFold,pSubFold,subFold,k-1,rep,verbose,p.trainInfo{1},p.testInfo{1});
        subFoldDecision(teInd,1) = curDecision;
        [hitRate(fold,1),curDecision,teInd,fData,curw,cura] = trainNtest5(dSubFold,pSubFold,subFold,k-1,rep,verbose,p.trainInfo{2},p.testInfo{2});
    ...
    [hitRate(fold,1),curDecision,teInd,fData,curw,cura] = trainNtest5(dOrig,p,fold,k,rep,verbose,p.trainInfo{1},p.testInfo{1});
    foldDecision(:,1) = curDecision;
    [hitRate(fold,1),curDecision,teInd,fData,curw,cura] = trainNtest5(dOrig,p,fold,k,rep,verbose,p.trainInfo{2},p.testInfo{2});
    foldDecision(:,2) = curDecision;
...
function [hitRate,decision_values,teInd,filterdData,w,a] = trainNtest5(dOrig,p,fold,k,rep,verbose,trainInfo,testInfo)
%% Train
% Prepare train data
if ~strcmp(p.k,'sess')
...
function [hitRate,decision_values,teInd,filterdData,w,a] = trainNtest6(dOrig,p,fold,k,rep,verbose,trainInfo,testInfo,subFoldDecision)
%% Train
% Prepare train data
if ~strcmp(p.k,'sess')
    [tr,~,teInd1,teInd2] = prepareDataAtFold(dOrig,p,trainInfo,fold,k,rep,verbose);
