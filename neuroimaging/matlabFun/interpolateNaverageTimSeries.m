function curResp = interpolateNaverageTimSeries(splitData_detrended,delay,grandRefDelay,runNum,runList,sess,subjInd,tList)
...
for x = 1:size(splitData_detrended{runInd},1)
    for y = 1:size(splitData_detrended{runInd},2)
        for z = 1:size(splitData_detrended{runInd},3)
            if any(isnan(delay{subjInd}(x,y,z,sess,:)))
                ...
                t0 = 0:length(tmpTs)-1;
                tmpTs = [tmpTs(1:12); tmpTs; tmpTs(end-11:end)];
