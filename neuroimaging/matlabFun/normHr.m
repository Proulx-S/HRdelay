...
% Complexify Hr
hrC = complex(hr,circshift(hr,size(hr,1)/4,1));
% Shift and scale Hr
thetaShift = angle(mean(hrSin,3)) - angle(mean(mean(hrSin,3),2));
rhoScale = abs(mean(hrSin,3)) ./ abs(mean(mean(hrSin,3),2));
theta = angle(hrC) - thetaShift;
rho = abs(hrC) ./ rhoScale;
% Reconstruct Hr
[hr,~] = pol2cart(theta,rho);
