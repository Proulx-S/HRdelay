function [pcaRes,predRes] = runRespPCA(dOrig,p,verbose)
...
%Do PCA on whole data set
xData = cat(1,mean(d.x(d.y==1,:,:),1),mean(d.x(d.y==2,:,:),1));
xData = permute(cat(3,xData(1,:,:),xData(2,:,:)),[3 2 1]);
xData_shift = mean(xData,1);
xData = xData - repmat(xData_shift,[size(xData,1) 1]);
xData_scale = std(xData,1);
xData = xData./repmat(xData_scale,[size(xData,1) 1]);

% [pcaRes.coeff,pcaRes.score,pcaRes.latent,~,~,~] = pca(zscore(xData,0,1),'centered',0); %coeff: run x pcTime x vox
[pcaRes.coeff,pcaRes.score,pcaRes.latent,~,~,~] = pca(xData,'centered',0); %coeff: run x pcTime x vox
% plotPCA(pcaRes,p)
%Pad with nan to full rank
pcaRes.coeff = cat(2,pcaRes.coeff,nan(size(xData')-size(pcaRes.coeff)));
pcaRes.score = cat(2,pcaRes.score,nan([size(xData,1) size(xData,1)]-size(pcaRes.score)));
pcaRes.latent = cat(2,pcaRes.latent,nan([size(xData,1) 1]-size(pcaRes.latent)));
...
% pcInd = 4;
% plot([scoreOri1(:,pcInd) scoreOri2(:,pcInd)])
% %%%%%%%%%%%%%%%%%%%
% %%%%%%%%%%%%%%%%%%%
% %%%%%%%%%%%%%%%%%%%
