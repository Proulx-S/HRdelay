...
results.OLS.mixed.runInd = find(any(results.OLS.mixed.designmatrixPieces.cond,1));
results.OLS.mixed.runInd = reshape(results.OLS.mixed.runInd,[2 length(results.OLS.mixed.runInd)/2])';
if opt.wantpercentbold
    for i = 1:size(results.OLS.mixed.constant.brain,4)
        con = 1./abs(results.OLS.mixed.constant.brain(:,:,:,i)) * 100;
        results.OLS.mixed.parameters(:,:,:,results.OLS.mixed.runInd(i,:)) = bsxfun(@times,results.OLS.mixed.parameters(:,:,:,results.OLS.mixed.runInd(i,:)),con);
    end
end
