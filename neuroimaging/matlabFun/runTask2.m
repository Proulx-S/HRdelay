            ...
            case 'SinCos'
                ...
                for cond = 1:3
                    ...
                    elseif ischar(p.split)
                        load(fullfile(dataDirIn,'run1a_preprocessing',['eqSess_' p.preSelection 'SinCos_' p.split '_' p.motionParam '_cond' num2str(cond) '.mat']),'results')
                    end
...
%                     [results.parametric.delayse, results.parametric.ampse] = cart2pol(results.parametric.parametersse(:,:,:,1),results.parametric.parametersse(:,:,:,2));
                    ...
                    allFields = {'amp' 'ampse' 'delay' 'F' 'Fp' 'Fz'};
                    for i = 1:length(allFields)
                        switch subj
                            case '01ha'
                                results.parametricRun.(allFields{i})(:,:,:,[7 11]) = [];
                            case '02jp'
                                results.parametricRun.(allFields{i})(:,:,:,6) = [];
                            case '03sk'
                            case '04sp'
                                results.parametricRun.(allFields{i})(:,:,:,end) = [];
                            case '05bm'
                            case '06sb'
                            case '07bj'
                        end
                    ...
                    switch subj
                        case '01ha'
                            results.parametricRun.constant.brain(:,:,:,[7 11]) = [];
                            results.inputs.datasize([7 11]) = [];
                            results.inputs.design([7 11]) = [];
                            results.R2run(:,:,:,[7 11]) = [];
                            results.pcregressors([7 11]) = [];
                        case '02jp'
                            results.parametricRun.constant.brain(:,:,:,6) = [];
                            results.inputs.datasize(6) = [];
                            results.inputs.design(6) = [];
                            results.R2run(:,:,:,6) = [];
                            results.pcregressors(6) = [];
                        case '03sk'
                        case '04sp'
                            results.parametricRun.constant.brain(:,:,:,end) = [];
                            results.inputs.datasize(end) = [];
                            results.inputs.design(end) = [];
                            results.R2run(:,:,:,end) = [];
                            results.pcregressors(end) = [];
                        case '05bm'
                        case '06sb'
                        case '07bj'
                    end
...
% imagesc(tmp)
                ...
                switch subj
                        case '01ha'
                            ind2rm = [7 11];
                            tmpInd = false(length(tmp.runList)/3,1);
                            tmpInd(ind2rm) = true;
                            ind2rm = false(length(tmp.runList),1);
                            ind2rm(1:3:end) = tmpInd;
                            ind2rm(2:3:end) = tmpInd;
                            ind2rm(3:3:end) = tmpInd;
                            tmp.runList(ind2rm) = [];
                            tmp.run(ind2rm) = [];
                            tmp.trial(ind2rm) = [];
                        case '02jp'
                ...
                switch subj
                        case '01ha'
                            ind2rm = [7 11];
                            tmpInd = false(length(runList)/3,1);
                            tmpInd(ind2rm) = true;
                            ind2rm = false(length(runList),1);
                            ind2rm(1:3:end) = tmpInd;
                            ind2rm(2:3:end) = tmpInd;
                            ind2rm(3:3:end) = tmpInd;
                            runList(ind2rm) = [];
