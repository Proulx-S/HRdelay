...
%     teInd{3}
    ...
    for i = 1:length(sessions)
        curInd = find(p2.sessionLabel(:,:,1)==sessions(i));
...
%     [tr.delay,te.delay,tr.label,tr.sessionLabel,te.label,te.sessionLabel,tr.label1,tr.label2] = splitTrainAndTest(d,curFoldLabel,'delay');
    ...
    tmp = reshape(d2tr.amp,[size(d2tr.amp,1)*size(d2tr.amp,3) size(d2tr.amp,2)])';
    tmp = corr(tmp);
    imagesc(tmp)
    ...
    if ~exist('sortIndFix','var')
        switch p.keepInfo
            case 'm'
                curMI = computeMI(tr.amp,tr.label1,tr.label2);
