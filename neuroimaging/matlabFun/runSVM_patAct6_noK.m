function out = runSVM_patAct6_noK(d,p,verbose,rep)
...
if p.doPerm
    if ischar(p.k)
        switch p.k
            ...
            case {'none'}
                runLabel = 1:length(d.label)/str2num(p.split(1));
                runLabel = runLabel(randperm(length(runLabel)));
                runLabel = reshape(repmat(runLabel,str2num(p.split(1)),1),1,length(d.label))';
                [~,b] = sort(runLabel);
                d.label = d.label(b,1);
            otherwise
