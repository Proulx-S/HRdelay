function [ output_args ] = runTask(allFile,altPerm,altMainDir)
...
%         imagesc(tmp)
        ...
        if stopThis
            ...
            if plotIt
                ...
                if svm.p.doSVM
                    saveas(h,fullfile(dataDirOut,[dataFileOut '1.jpg']),'jpg')
                end
                if svm.p.doCrossVal
                    saveas(h2,fullfile(dataDirOut,[dataFileOut '2.jpg']),'jpg')
                end
            end
            %reduce size
            svmOrig = svm;
...
%         length(find(diff(a)==0))/length(a)
    ...
    if perm.doPerm
        ...
        while i<perm.numPermPerBloc && ~permDone
            if svm.p.doSVM
                allHitRate = nan(perm.compileEachNperm,length(altFeatLev));
            end
            if svm.p.doCrossVal
                allRsquared = nan(perm.compileEachNperm,length(altFeatLev));
            end
...
%                 %the old way respecting a lot of constraints
%                 [curd,tmpp] = handlePermutation(d,p);

%                 %shuffling all 3 labels, in order to account for
%                 %normalization (which occur in runSVMRepetitions)
%                 keyboard
%                 curd = d; tmpp = p;
%                 tmpData = cat(3,curd.xData(1:end/2,:),curd.xData(end/2+1:end,:),curd.normData);
%                 tmpData = permute(tmpData,[3 1 2]);
%                 tmpData = Shuffle(tmpData,3);
%                 tmpData = permute(tmpData,[2 3 1]);
%                 curd.xData(1:end/2,:) = tmpData(:,:,1);
%                 curd.xData(end/2+1:end,:) = tmpData(:,:,2);
%                 curd.normData = tmpData(:,:,3);
%                 clear tmpData
...
%                 %shuffling 2 labels
%                 curd = d; tmpp = p;
%                 tmpData = cat(3,curd.xData(1:end/2,:),curd.xData(end/2+1:end,:));
%                 tmpData = permute(tmpData,[3 1 2]);
%                 tmpData = Shuffle(tmpData,3);
%                 tmpData = permute(tmpData,[2 3 1]);
%                 curd.xData(1:end/2,:) = tmpData(:,:,1);
...
%                 clear tmpData
                ...
                %shuffling 2 labels (keep session order)
                curd = d; tmpp = p;
                tmpData = curd.xData;
                shufInd = [ones(size(tmpData,1)/2,1);ones(size(tmpData,1)/2,1)*2];
                shufInd = shufInd(randperm(length(shufInd)));
                curd.xData(1:end/2,:) = tmpData(shufInd==1,:);
                curd.xData(end/2+1:end,:) = tmpData(shufInd==2,:); clear shufInd
...
%                 [curd,tmpp] = handlePermutation(d,p);
%                 [curcurd,curtmpp] = handleKfolding(curd,tmpp);
%                 assessCriteria(curcurd,curtmpp)
%                 [curd,tmpp] = handlePermutation(d,p);
%                 [curcurd,curtmpp] = handleKfolding(curd,tmpp);
%                 curcurd.label'
%                 [curd,tmpp] = handlePermutation(d,p);
%                 [curcurd,curtmpp] = handleKfolding(curd,tmpp);
%                 curcurd.sessionLabel'
%                 [curd,tmpp] = handlePermutation(d,p);
%                 [curcurd,curtmpp] = handleKfolding(curd,tmpp);
%                 curcurd.crossVal'
%                 
...
%                 curcurd.crossVal'
%                 
%                 %%double-check if decent amount of unique permutation
%                 for iiii = 1:500
%                     [curcurd,curtmpp] = handleKfolding(curd,tmpp);
%                     allCrossVall(:,iiii) = curcurd.crossVal';
%                 end
%                 allCrossVall = relabelCrossVal(allCrossVall);
%                 size(allCrossVall)
%                 allCrossVallUnique = unique(allCrossVall','rows')';
%                 size(allCrossVallUnique)
%                 size(allCrossVallUnique)/size(allCrossVall)
% 
%                 for iiii = 1:500
%                     [curd,tmpp] = handlePermutation(d,p);
%                     allCrossVall(:,iiii) = curd.label';
%                 end
%                 allCrossVall = relabelCrossVal(allCrossVall);
%                 size(allCrossVall)
%                 allCrossVallUnique = unique(allCrossVall','rows')';
...
%                 size(allCrossVallUnique)/size(allCrossVall)
                ...
                keyboard
                [cursvm,curRsquared] = runSVMRepetitions(curd,tmpp,0);
...
%                 allRsquared(ii,:) = mean(curRsquared,1);
            ...
            %% Compile
            r.hitRate = compilePermRes(r.hitRate,svm.p.featLevList,allHitRate,altFeatLev);
%             xValCorr.Rsquared = compilePermRes(xValCorr.Rsquared,svm.p.featLevList,allRsquared,altFeatLev);
            ...
            svm.pP = p;
            svm.rP = r;
%             svm.xValCorrP = xValCorr;
            
            % Compute non-param thresh so far
            threshAcc = computeNonParmThresh(svm.rP.hitRate,[2.5 5 95 97.5]);
            svm.rP.thresh = threshAcc;
%             threshAcc = computeNonParmThresh(svm.xValCorrP.Rsquared,[2.5 5 95 97.5]);
%             svm.xValCorrP.thresh = threshAcc;
            ...
            if p.doSVM
                saveas(h1,fullfile(p.dataDirOut,[p.dataFileOut '1.jpg']),'jpg')
            end
            if p.doCrossVal
                saveas(h2,fullfile(p.dataDirOut,[p.dataFileOut '2.jpg']),'jpg')
            end
